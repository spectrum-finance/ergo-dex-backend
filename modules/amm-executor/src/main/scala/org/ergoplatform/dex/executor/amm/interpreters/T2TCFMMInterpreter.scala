...
final class T2TCFMMInterpreter[F[_]: Monad: ExecutionFailed.Raise](
...
) extends CFMMInterpreter[T2T_CFMM, InterpreterVersion.V1, F] {

  val helpers = new CFMMInterpreterHelpers(exchange, monetary)
  import helpers._

  def deposit(in: AnyDeposit, pool: CFMMPool): F[(ErgoLikeTransaction, Traced[Predicted[CFMMPool]])] = in match {
  ...
  def redeem(in: AnyRedeem, pool: CFMMPool): F[(ErgoLikeTransaction, Traced[Predicted[CFMMPool]])] = in match {
...
object T2TCFMMInterpreter {
  ...
  def make[I[_]: Functor, F[_]: Monad: ExecutionFailed.Raise: ExchangeConfig.Has: MonetaryConfig.Has](implicit
    ...
    encoder: ErgoAddressEncoder,
    logs: Logs[I, F]
  ): I[CFMMInterpreter[T2T_CFMM, InterpreterVersion.V1, F]] =
    logs.forService[CFMMInterpreter[T2T_CFMM, InterpreterVersion.V1, F]].map { implicit l =>
      (
        for {
          exchange   <- ExchangeConfig.access
          execution  <- MonetaryConfig.access
          networkCtx <- NetworkContext.make
        } yield new CFMMInterpreterTracing[T2T_CFMM, InterpreterVersion.V1, F] attach new T2TCFMMInterpreter(
          exchange,
          execution,
          networkCtx
        )
