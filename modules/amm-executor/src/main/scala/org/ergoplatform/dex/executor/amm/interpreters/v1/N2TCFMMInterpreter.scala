...
import org.ergoplatform.dex.executor.amm.interpreters.v1.InterpreterV1.InterpreterV1Tracing
  ...
  def make[I[_]: Functor, F[_]: Monad: ExecutionFailed.Raise: ExchangeConfig.Has: MonetaryConfig.Has](implicit
    ...
    logs.forService[InterpreterV1[N2T_CFMM, F]].map { implicit l =>
      (
        ...
        } yield new InterpreterV1Tracing[N2T_CFMM, F] attach new N2TCFMMInterpreter(
          exchange,
          execution,
          networkCtx
        )
