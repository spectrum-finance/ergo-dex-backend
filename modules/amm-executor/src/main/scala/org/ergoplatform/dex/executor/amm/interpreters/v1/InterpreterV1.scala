...
trait InterpreterV1[CT <: CFMMType, F[_]] {
  ...
  def deposit(deposit: DepositErgFee, pool: CFMMPool): F[(ErgoLikeTransaction, Traced[Predicted[CFMMPool]], Output)]

  def redeem(redeem: RedeemErgFee, pool: CFMMPool): F[(ErgoLikeTransaction, Traced[Predicted[CFMMPool]], Output)]

  def swap(swap: SwapErg, pool: CFMMPool): F[(ErgoLikeTransaction, Traced[Predicted[CFMMPool]], Output)]

}

object InterpreterV1 {

  final class InterpreterTracing[CT <: CFMMType, F[_]: FlatMap: Logging](orderType: String) extends InterpreterV1[CT, Mid[F, *]] {
    ...
    def swap(swap: SwapErg, pool: CFMMPool): Mid[F, (ErgoLikeTransaction, Traced[Predicted[CFMMPool]], Output)] =
