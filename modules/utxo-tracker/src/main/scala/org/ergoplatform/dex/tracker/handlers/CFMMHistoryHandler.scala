...
import org.ergoplatform.dex.domain.amm.{CFMMOrder, CFMMPool, CFMMVersionedOrder, EvaluatedCFMMOrder, OrderEvaluation, OrderId, PoolId}
import org.ergoplatform.dex.protocol.amm.AMMType.{CFMMType, N2T_CFMM, T2T_CFMM}
import org.ergoplatform.dex.tracker.parsers.amm.CFMMHistoryVersionedParser._
import org.ergoplatform.dex.tracker.parsers.amm.{CFMMHistoryVersionedParser, CFMMOrdersParser, CFMMPoolsParser, VersionedCFMMParser}
...
final class CFMMHistoryHandler[
  ...
  G[_]: Monad: Logging
                                                          ...
                                                          producer: Producer[OrderId, EvaluatedCFMMOrder[CFMMVersionedOrder, OrderEvaluation], F]
) {
  ...
  def handler: SettledTxHandler[F] =
    _.evalMap { tx =>
      parsers
        .traverse { parser =>
          def widen[A <: CFMMVersionedOrder, E <: OrderEvaluation](eo: EvaluatedCFMMOrder[A, E]) =
            eo.copy(order = eo.order: CFMMVersionedOrder, eval = eo.eval.widen[OrderEvaluation])
          for {
      ...
      .map(op => Record[OrderId, EvaluatedCFMMOrder[CFMMVersionedOrder, OrderEvaluation]](op.order.id, op))
