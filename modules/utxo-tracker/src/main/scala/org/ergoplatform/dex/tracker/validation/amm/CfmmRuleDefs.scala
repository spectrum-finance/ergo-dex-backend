  ...
  private def sufficientValueSwap: CFMMRule = { order =>
    (order match {
      ...
      case Some((params, isTokenFee, maxMinerFee, box, reservedExFee)) =>
        ...
        val insufficientFeeErg   = checkFee(maxDexFeeNetErg, false)
        val insufficientFeeToken = checkFee(maxTokenFee, true)
        insufficientFeeErg orElse insufficientFeeToken orElse insufficientValue
      case None => None
    }
