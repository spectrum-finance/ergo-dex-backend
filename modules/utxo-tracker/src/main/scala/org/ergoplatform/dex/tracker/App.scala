...
import org.ergoplatform.dex.tracker.configs.ConfigBundle._
...
object App extends EnvApp[ConfigBundle] {
  ...
  private def init(configPathOpt: Option[String]): Resource[InitF, (UtxoTracker[StreamF], ConfigBundle)] =
    for {
      ...
      tracker <-
        Resource.eval(UtxoTracker.make[InitF, StreamF, RunF](TrackerMode.Live, t2tCfmmHandler, cfmmPoolsHandler))
