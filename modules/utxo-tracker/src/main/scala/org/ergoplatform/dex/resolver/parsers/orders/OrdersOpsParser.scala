package org.ergoplatform.dex.resolver.parsers.orders
...
import org.ergoplatform.dex.resolver.domain.errors._
...
object OrdersOpsParser {
  ...
  final private class Live[F[_]: Monad: Clock: Raise[*[_], InvalidOrder]](
    ...
    private[resolver] def isSellerScript(tree: ErgoTree): Boolean =
      util.Arrays.equals(tree.template, sellerContractErgoTreeTemplate)

    private[resolver] def isBuyerScript(tree: ErgoTree): Boolean =
      util.Arrays.equals(tree.template, buyerContractErgoTreeTemplate)

    private[resolver] def makeAsk(tree: ErgoTree, output: Output): F[Ask] =
      for {
    ...
    private[resolver] def makeBid(tree: ErgoTree, output: Output): F[Bid] =
      for {
