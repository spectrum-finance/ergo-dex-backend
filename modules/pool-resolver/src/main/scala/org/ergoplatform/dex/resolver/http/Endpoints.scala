...
import org.ergoplatform.dex.domain.amm.state.OffChainIndexed
...
object Endpoints {
  ...
  def putPredicted: Endpoint[OffChainIndexed[CFMMPool], HttpError, Unit, Any] =
    endpoint.post
      .in("predicted")
      .in(jsonBody[OffChainIndexed[CFMMPool]])
